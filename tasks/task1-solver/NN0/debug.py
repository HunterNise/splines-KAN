...
## 0-int-1 or int: num_knots=5; all: num_knots=11
model = NN(num_knots=11).to(device)  # create model instance and move to device
...
def train(model, points, degree, param="uniform",
    ...
    for iter in range(2):
        ...
        ## 0-int-1: model.num_knots; int: model.num_knots-2, initial_knots[1:-1]; all: model.num_knots - 2*degree, cat(zeros(degree), initial_knots, ones(degree))
        initial_knots = torch.linspace(0., 1., model.num_knots - 2*degree, 
                                       dtype=points.dtype, device=points.device)
        #initial_knots = initial_knots[1:-1]   # remove the first and last knot for the model input (since they are fixed at 0 and 1)
        ...
        #              ))
        ## all: no cat
        full_knots = pred_knots # since the model already outputs the full knot vector with multiplicity due to the way we set up the input and softmax, we can use pred_knots directly as full_knots
        print(f"Full knots:\n{full_knots}")         ###
        
        # compute the B-spline basis matrix evaluated at the parameter grid for the predicted knots and given degree
        basis_matrix = bspline_basis(t_grid, full_knots, degree, soft=True)
        ...
        print(f"Loss value: {loss.item():.6e}")     ###
        ...
        for name, param in model.named_parameters():
            if param.grad is not None:
                ...
                print(f"  {name}: norm={grad_norm:.6e}, min={grad_min:.6e}, max={grad_max:.6e}, NaN={has_nan}")
            else:
