"""
[task1/KAN2] Dataset-level B-spline knot predictor — KAN version, GPU-optimised.

This file mirrors train.py but replaces all per-sample Python loops with fully batched
tensor operations, identical in spirit to task1/NN2/train_gpu.py.  The produced model
file, loss curves, and training-info text are identical in format to those from train.py,
so the same eval.py can be used without modification.
...
2. No DataLoader: the training loop uses torch.randperm + direct tensor slicing,
   ...
   update_grid_from_samples reads directly from train_pts (already on device) instead.
...
5. Safe division with torch.where: replaces the boolean-indexed scatter used in
   functions.bspline_basis_matrix with a branch-free elementwise select, avoiding
   non-contiguous GPU memory access patterns.
...
6. Pre-allocated buffers: the knot-clamping zero/one pad tensors and the regularisation
   ...
   per-batch / per-sample iteration.
...
8. model.speed() — pykan overhead elimination: by default pykan's MultKAN.forward()
   runs with save_act=True and symbolic_enabled=True.  save_act=True stores
   spline_preacts, spline_postacts, spline_postsplines, and acts_premult and computes
   four torch.std() calls per layer on every forward pass (purely for visualisation).
   symbolic_enabled=True evaluates the symbolic branch even though all symbolic
   functions are zero during training.  model.speed() sets both flags to False and
   also disables auto_save, removing all this overhead with a single call.
   update_grid_from_samples() calls get_act() internally, which temporarily restores
   save_act=True for its own pass, so grid updates work correctly without any extra
   handling.

KAN-specific notes
------------------
- The KAN forward pass returns raw (unbounded) interval logits; softmax + cumsum are
  applied in the training loop (same as train.py) to produce knots in [0, 1].
- model.speed() is called once after model creation; it persists for the entire run.
- grid_update_interval works as before: every N epochs, update_grid_from_samples
  refines the KAN spline grids (it calls get_act() internally, no manual warm-up
  needed).
"""
...
class BSplineDataset:
    ...
    - Does NOT subclass torch.utils.data.Dataset: the training loop uses direct
      tensor slicing instead of a DataLoader, so the DataLoader API is not needed.
...
# Define the KAN model (architecture unchanged from train.py)
...
# GPU optimisation: disable pykan's visualisation/symbolic overhead for the entire run.
# By default MultKAN.forward() runs with save_act=True (stores spline_preacts,
# spline_postacts, spline_postsplines, acts_premult and calls 4 torch.std() per layer
# on every forward pass) and symbolic_enabled=True (evaluates the symbolic branch even
# though no symbolic functions are fitted during training).  model.speed() sets both
# flags to False and also disables auto_save — pykan's own mechanism for this purpose.
# update_grid_from_samples() calls get_act() internally, which temporarily re-enables
# save_act for its own single pass, so grid updates work correctly without any change.
# To also apply torch.compile, use model.speed(compile=True) instead.
model.speed()
...
def kan_forward_to_knots(model, pts_batch):
    ...
    zero               = torch.zeros(pts_batch.shape[0], 1,
                                     dtype=pred_intervals.dtype,
                                     device=pred_intervals.device)
...
def train(model, train_pts, train_knots, test_pts, test_knots,
    ...
    Changes vs train.py
    --------------------
    - No DataLoader: uses torch.randperm on-device + direct tensor slicing.  All data
      is already resident on the device, so there is no per-batch host-to-device transfer.
    - Inner for-loop over batch samples replaced by bspline_basis_matrix_batched +
      solve_control_points_batched.
    - model.speed() called once after model creation disables save_act and
      symbolic_enabled for all forward passes; update_grid_from_samples() re-enables
      save_act internally for its own pass and restores it afterwards.
    - Grid update no longer requires a separate reinit_kan warm-up pass;
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # update_grid_from_samples() calls get_act() internally, which temporarily
        # restores save_act=True for its own forward pass, so no manual warm-up is
        # needed.  Data is already on device — no host-to-device transfer here.
        if grid_update_interval > 0 and epoch > 0 and epoch % grid_update_interval == 0:
            model.update_grid_from_samples(train_pts[:batch_size])
        ...
        for i in tqdm(range(0, n_train, batch_size), ncols=100, desc=f"Epoch {epoch}"):
            ...
            # KAN forward pass + softmax + cumsum to produce knots in [0, 1]
            pred_knots = kan_forward_to_knots(model, points_batch)   # (B, num_knots)
